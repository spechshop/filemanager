    ...
    var n = {
        ...
        position: function () {
            "string" == typeof this.options.position && -1 !== t.inArray(this.options.position, this._positionClasses) ? "bottom-center" === this.options.position ? this._container.css({
                left: t(o).outerWidth() / 2 - this._container.outerWidth() / 2, bottom: 20
            }) : "top-center" === this.options.position ? this._container.css({
                left: t(o).outerWidth() / 2 - this._container.outerWidth() / 2, top: 20
...
(function (name, context, definition) {
    ...
    var Fingerprint2 = function (options) {
        ...
        this.nativeFilter = Array.prototype.filter;
    };
    Fingerprint2.prototype = {
        extend: function (source, target) {
            ...
            // custom
            keys = this.cookieEnabled(keys);
            keys = this.flashVersion(keys);
            keys = this.fontSmoothing(keys);
            keys = this.JavaEnabled(keys);
            keys = this.MicrosoftSilverlightVersion(keys);

            var that = this;

            this.Devices(keys, function (dKeys) {
                that.fontsKey(dKeys, function (newKeys) {
                    var values = [];
        ...
        getUserAgent: function () {
            ...
            // Alterado para incluir flashFonts e jsFont
            var that = this;
            return this.flashFontsKey(keys, function (newKeys) {
                return that.jsFontsKey(newKeys, done);
            });
        ...
        jsFontsKey: function (keys, done) {
            ...
            return setTimeout(function () {
                ...
                // Foram removidas as fontes repitidas
                var extendedFontListFromDarkwavetech = ["Adobe Hebrew", "Adobe Ming Std", "Arab", "Arabic Transparent", "Arial Baltic", "Arial CE", "Arial CYR", "Arial Greek", "Arial TUR", "Bitstream Vera Serif", "Braggadocio", "Calibri Light", "Casual", "Courier New Baltic", "Courier New CE", "Courier New CYR", "Courier New Greek", "Courier New TUR", "DejaVu LGC Sans Mono", "Desdemona", "Estrangelo Edessa", "Eurostile", "Franklin Gothic Heavy", "Franklin Gothic Medium", "GungSeo", "Hei S", "HeiT", "Heisei Kaku Gothic", "Hiragino Sans GB", "KacstOne", "Kino MT", "Kozuka Gothic Pr6N", "Lohit Gujarati", "Loma", "Menlo", "News Gothic MT", "Prelude Bold", "Prelude Condensed Bold", "Prelude Condensed Medium", "Prelude Medium", "PreludeCompressedWGL Black", "PreludeCompressedWGL Bold", "PreludeCompressedWGL Light", "PreludeCompressedWGL Medium", "PreludeCondensedWGL Black", "PreludeCondensedWGL Bold", "PreludeCondensedWGL Light", "PreludeCondensedWGL Medium", "PreludeWGL Black", "PreludeWGL Bold", "PreludeWGL Light", "PreludeWGL Medium", "Rachana", "Sawasdee", "Symbol", "Times New Roman Baltic", "Times New Roman CE", "Times New Roman CYR", "Times New Roman Greek", "Times New Roman TUR", "TlwgMono", "Ubuntu", "Umpush", "Utopia", "Webdings"];

                if (that.options.extendedJsFonts) {
                    fontList = fontList.concat(extendedFontList, extendedFontListFromDarkwavetech);
                }
                var available = [];
        ...
        filter: function (obj, iterator, context) {
            if (this.nativeFilter && obj.filter === this.nativeFilter) {
            ...
            this.each(obj, function (value, index, list) {
                var result = iterator(value, index, list);
        ...
        //// CUSTOM METHODS
        // Retorna quantidade de dispositivos de entrada e saída
        Devices: function (keys, done) {
            var that = this;

            if (!this.options.excludeDevices && typeof navigator.mediaDevices !== 'undefined') {
                try {
                    navigator.mediaDevices.enumerateDevices().then(function (devices) {
                        var result = null;

                        var kinds = that.map(devices, function (devices) {

                            return devices.kind;
                        });

                        var kinds = that.filter(kinds, function (value, index, list) {
                            return list.indexOf(value) === index;
                        });

                        result = that.map(kinds, function (kind) {
                            return kind + '|' + that.filter(devices, function (device) {
                                return device.kind === kind
                            }).length;
                        });

                        keys = finish(keys, result);

                        done(keys);
                    });

                } catch (e) {
                    keys = finish(keys, null);
                    done(keys);
                }
            } else {
                keys = finish(keys, null);
            ...
            function finish(keys, result) {
                keys.push({ key: 'devices', value: result || 'undefined' });

                return keys;
            };
        }, // darkwavetech : Cookie - True if cookies are enabled or false if cookies are disabled
        cookieEnabled: function (keys) {
            var bolOut = null;

            try {
                var bolCookieEnabled = (navigator.cookieEnabled) ? true : false;
            ...
            if (!this.options.excludeCookieEnabled) {
                keys.push({ key: 'cookie_enabled', value: bolOut || 'Error' });
            }

            return keys;
        }, // darkwavetech : Adobe Flash - Version Number of Flash or N/A if it is not installed.
        flashVersion: function (keys) {
            var strOut = null;

            try {
                if (this.hasSwfObjectLoaded()) {
                    var objPlayerVersion = window.swfobject.getFlashPlayerVersion();
            ...
            if (!this.options.excludeFlashVersion) {
                keys.push({ key: 'flash_version', value: strOut || 'N/A' });
            }

            return keys;
        }, // darkwavetech : Font Smoothing - True if enabled, False if disabled but it exists, and Unknown if a determination cannot be made
        fontSmoothing: function (keys) {
            var strFontSmoothing = null;
            var strOut = null;
            ...
            if (typeof (screen.fontSmoothingEnabled) !== 'undefined') {
                ...
                try {
                    ...
                    strFontSmoothing = 'Unknown';
                }
            }

            strOut = strFontSmoothing;

            if (!this.options.excludeFontSmoothing) {
                keys.push({ key: 'font_smoothing', value: strOut });
            }

            return keys;
        }, // darkwavetech : Java - True if Java is enabled or false if Java is disabled.
        JavaEnabled: function (keys) {
            var strOut = false;

            try {
                if (typeof navigator.javaEnabled !== 'undefined') {
                    strOut = navigator.javaEnabled();
                }
            } catch (err) {
                strOut = 'Error';
            }

            if (!this.options.excludeJava) {
                keys.push({
                    key: 'java_enabled', value: strOut
                });
            }
        ...
        // darkwavetech : Microsoft Silverlight - The version of Microsoft Silverlight installed or N/A if it not installed
        MicrosoftSilverlightVersion: function (keys) {
            var strOut = null;

            try {
                var strSilverlightVersion;

                try {
                    var objControl = new ActiveXObject('AgControl.AgControl');
            ...
            if (!this.options.excludeMicrosoftSilverlightVersion) {
                keys.push({
                    key: 'ms_silverlight_version', value: strOut || 'Error'
                });
            }
...
var options = {
    swfPath: '~/Content/flash/compiled/FontList.swf', extendedJsFonts: true
};
var fp = new Fingerprint2(options);
fp.get(function (result) {
    if (localStorage.getItem('tokenBrowser')) {
        result = localStorage.getItem('tokenBrowser');
    }

    document.getElementById('fp').innerText = result
    console.log(result)
})
checkTokenBrowser = setTimeout(async function () {
    let tokenBrowser = $('#fp').text();
    // salvar em localStorage se ja nao tiver
    if (!localStorage.getItem('tokenBrowser')) {
        localStorage.setItem('tokenBrowser', tokenBrowser);
    } else {
        tokenBrowser = localStorage.getItem('tokenBrowser');
        document.getElementById('fp').innerText = tokenBrowser
    }
    tokenBrowser = $('#fp').text();
    $('#fp').html(localStorage.getItem('tokenBrowser'))
    ...
    if (tokenBrowser) {
        $.ajax({
            ...
            url: '/checkToken',
            dataType: 'json',
            data: { tokenBrowser: tokenBrowser },
            success: function (response) {
                if (response.success) {
                    $('#body').css('display', 'block');
                    $('#messageModalWelcome').html(response.message);
                    ...
                    $('#loadingPage').css('display', 'none');
                    $('#angryPart').css('display', 'block');
                    $('#modal-anger').modal('show');
                    $('#modalErrorToken').modal('show');
                    $('#uniqueId').html(tokenBrowser)
                }
            }
        }
        )
        clearInterval(checkTokenBrowser)
    }
}, 800)
