...
function curlToPHP(curl) {
    ...
    /**
     * @param {{headers: *[], method: string, data: {}, url: string}} req
     */
    function renderComplex(req) {
        ...
        // complete request
        php += '\n' + result + '\n' + err + endCurl
        $('#url-import').text(req.url)
        let splitHeader;
        $('#httpHeadList').html('')

        req.headers.forEach(function (headerString, idHeader) {
            splitHeader = headerString.replaceAll(' : ', ':').split(/:(.+)/)
            $('#httpHeadList').append(`<div class="input-group mb-1" id="headerInfo${idHeader}">
                            <input name="keyHeader[${idHeader}]" type="text" class="form-control" placeholder="Chave" value="${splitHeader[0].trim()}" aria-label="Chave">
                            <span class="input-group-text">:</span>
                            <input name="valueHeader[${idHeader}]" type="text" class="form-control" placeholder="Valor" value="${splitHeader[1].trim()}" aria-label="Valor">
                            <button onclick="createRowHeader()" class="btn btn-blue-sky" type="button"><i class="fa-solid fa-plus text-white"></i></button>
                            <button onclick="deleteRowHeader('headerInfo${idHeader}')" class="btn btn-danger" type="button"><i class="fa-solid fa-trash"></i></button>
                        </div>`)
        })
        $('#methodMakeCall').html(req.method)
        $('#payloadMake').text(req.data.ascii)

        return php;
    }

    /**
     *  var arr = $('input[name="keyHeader[0]"]').map(function () { return this.value; // $(this).val()  }).get();  console.log(arr)
     * */
...
function deleteRowHeader(idRow) {
    $(`#${idRow}`).remove()
}

function createRowHeader() {
    $('#httpHeadList').append(`<div class="input-group mb-1" id="headerInfo${$('#httpHeadList').children().length}">
                            <input name="keyHeader[${$('#httpHeadList').children().length}" type="text" class="form-control" placeholder="Chave" aria-label="Chave">
                            <span class="input-group-text">:</span>
                            <input name="valueHeader[${$('#httpHeadList').children().length}" type="text" class="form-control" placeholder="Valor" aria-label="Valor">
                            <button onclick="createRowHeader()" class="btn btn-blue-sky" type="button"><i class="fa-solid fa-plus text-white"></i></button>
                            <button onclick="deleteRowHeader('headerInfo${$('#httpHeadList').children().length}')" class="btn btn-danger" type="button"><i class="fa-solid fa-trash"></i></button>
                        </div>`)
}

function importCURL() {
    let curl = $('#command-curl').val()
    console.log(curl)
    curlToPHP(curl)

    // set prism
    let code = curlToPHP(curl)
    $('#rawHTTPinterface').text(code)
    Prism.highlightAll()

}

function changeMethod(method) {
    $('#methodMakeCall').text(method.innerText)
}

function parseParams() {
    let url = $('#url-import').text()
