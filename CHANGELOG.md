...
- The Codex Agent conversation history now uses a dedicated Claude/ChatGPT-style sidebar with a prominent new-chat action, recent conversation count, active-chat highlighting, and a responsive collapsible drawer instead of a compact `<select>` menu
- Codex Agent conversations are now global instead of being partitioned by the File Manager's current folder. The selected conversation survives folder navigation and reconnection, while each new turn follows the folder currently open in the explorer without losing the existing chat context
...
- Codex conversations can now follow the File Manager into an existing directory outside the installation root instead of failing with "A pasta selecionada não existe ou está fora do projeto"; each permission mode still applies its sandbox to the selected directory. A failed first-open history request is also emitted only once per panel opening instead of being duplicated by both Bootstrap lifecycle events
- Opening the Codex Agent panel for the first time no longer leaves the conversation list and messages empty until the panel is closed and reopened. The panel now tracks Bootstrap's opening state before connecting, retries on `shown.bs.offcanvas`, and coalesces concurrent history loads instead of relying on the late-added `show` CSS class
- Codex file citations without an explicit line now default to line 1 and open in the embedded editor instead of a separate browser tab
